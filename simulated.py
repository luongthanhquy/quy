...
def detailCost(schedule):
    cost = 0
    for t_number, teacher in enumerate(T, 1):
        print("Teacher:")
        for t, occupied in enumerate(teacher):
            if occupied == 1:
                continue
            else:
    ...
    for i, row in enumerate(schedule):
        for j, val in enumerate(row):
            if val != 0 and C[i][j] == 0:
                print('C', i, j, val)
                cost += 1
            if val == 0 and D[i][j] == 1:
                print('D', i, j, val)
                cost += 1
    return cost
...
def annealing():
    global tmp_schedule
    temperature, cooling_sense, prob, times = 10000, 3.5, 0, 1
    c0, c1 = cost(tmp_schedule), 0
    while True:
        newState = randomState()
        c1 = cost(newState)
        prob = pow(math.e, (c1 - c0) / temperature)
        choose = random.random() - prob
        temperature -= cooling_sense
        print('t: ', temperature, 'oldVal: ', c0, 'newVal: ', c1, 'prob: ', prob, 'choose: ', choose)
        if c1 == 0 or (c1 < c0) or (choose > 0):
            tmp_schedule, c0 = newState, c1
        if (temparature < 1):
            if c0 != 0:
                temparature, times = 10000, times + 1
                if times == 10:
                    print(T)
                    return tmp_schedule
...
def annealing_one_line():
    global tmp
    temperature, cooling_sense, prob, times = 100, 0.99, 0, 1
    newState = []
    ...
    while True:
            ...
            print(f"t: {temperature}, oldVal: {c0}, newVal: {c1}, prob: {prob}, choose: {choose}")
            if c1 == 0:
                tmp = newState
                print(times)
                return tmp
            elif c1 < c0 or choose > 0:
                tmp = newState
                c0 = c1
            if temperature <= 0.002 and c0 != 0:
                temperature, cooling_sense, times = 1, 0.001, times+1
                if times == 5:
                    return tmp

def do_it():
    global T, real_schedule, tmp
    ...
    while tmp_schedule:
        ...
        C.pop(0)
        D.pop(0)
        T[(tmp != 0) & (T[tmp - 1, np.arange(len(tmp))] != 0)] = 0
